...
{	
	...
	//若已填写基本信息
	$error=false;
	$_SESSION['studentname'] = "PartTwo".htmlspecialchars($_REQUEST['one1'],ENT_QUOTES);
    $result=executeQuery("select name from users2 where name ='".$_SESSION['studentname']."';");
	$r=mysql_fetch_array($result);
    if(is_null($r['name'])){
		$query="insert users2 (name) value('".$_SESSION['studentname']."');";
		...
		$query="delete from users2 where name = '".$_SESSION['studentname']."';";
		if(!executeQuery($query))
		{
		$error=true;
