...
<?php
	...
	require_once('db.php');
//显示第一个表的信息
	if(!$_REQUEST['iID']){die('请填写身份证号');}
	echo '<font style="color:red;font-weight:bold;">表一：</font><br/>------------------------------------------------------------------<br/>';
	$query="select formid from PartOne".$_REQUEST['iID']." where formid=1;";
	$result=executeQuery($query);
	if(!$result) die(mysql_error()); 
	if(mysql_fetch_row($result)){
		$question1=array('姓名','性别','年龄（周岁）','民族','体重（kg）','身高（cm）','Email','电话','身份证号','镶过金属牙或体内有金属异物','若是请详细说明');
		for($i=1;$i<=11;$i++){
			$query="select * from PartOne".$_REQUEST['iID']." where formid=1 and questionid=".$i.";";
			$result=executeQuery($query);
			if(!$result){die(mysql_error());}
...
//显示第二个表的信息
	...
	$query="select formid from PartOne".$_REQUEST['iID']." where formid=2;";
	$result=executeQuery($query);
	if(!$result) die(mysql_error()); 
	if(mysql_fetch_row($result)){
		...
		for($i=1;$i<=22;$i++){
			$query="select * from PartOne".$_REQUEST['iID']." where formid=2 and questionid=".$i.";";
			$result=executeQuery($query);
			if(!$result){die(mysql_error());}
			while($r=mysql_fetch_row($result)){
				if($r[3]==''){ $r[3]='此处没有填写';}
...
//显示第三个表的信息
	...
	$query="select formid from PartOne".$_REQUEST['iID']." where formid=3;";
	$result=executeQuery($query);
	if(!$result) die(mysql_error()); 
	if(mysql_fetch_row($result)){
		...
		for($i=1;$i<=20;$i++){
			$query="select * from PartOne".$_REQUEST['iID']." where formid=3 and questionid=".$i.";";
			$result=executeQuery($query);
			if(!$result){die(mysql_error());}
			while($r=mysql_fetch_row($result)){
				if($r[3]==''){$r[3]='此处没有填写';}
